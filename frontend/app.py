            ...
            if use_streaming:
                ...
                with st.container():
                    ...
                    try:
                        ...
                        logger.info(
                            f"[FRONTEND] 응답 상태 코드: {response.status_code}"
                        )
                        ...
                        for line in response.iter_lines(
                            ...
                            if not line:
                                logger.debug(
                                    f"[FRONTEND] 빈 라인 수신 (라인 {line_count})"
                                )
                                continue
                                ...
                                try:
                                    ...
                                    if status == "searching":
                                        ...
                                        if animation_stop:
                                            animation_stop.set()
                                        # 새로운 애니메이션 시작
                                        base_message = safe_message.replace(
                                            "중...", "중"
                                        )
                                        animation_stop = (
                ...
                with show_loading("AI 답변 생성 중..."):
                    try:
                        ...
                        response = call_api(
                            "/chat", {"query": prompt, "llm_data": llm_data}
                        )
