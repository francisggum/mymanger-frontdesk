...
if st.sidebar.button("플랜 조회", type="primary"):
    if st.session_state.jwt_token:
        with show_loading("플랜 목록 조회 중..."):
            try:
                # 백엔드 API 호출
                response = call_api("/fetch-plans", {"jwt_token": st.session_state.jwt_token})
...
if "plans" in st.session_state and st.session_state.plans:
    ...
    # 플랜 선택
    plan_options = {plan["plan_name"]: plan["plan_id"] for plan in st.session_state.plans}
    ...
    # 성별 선택
    gender = st.sidebar.radio("성별", ["남성", "여성"], format_func=lambda x: "M" if x == "남성" else "F")
...
if not backend_available:
    st.error("⚠️ 백엔드 서버에 연결할 수 없습니다. 백엔드 서버가 실행 중인지 확인해주세요.")
    st.stop()
...
if "data_loaded" not in st.session_state:
    st.info("👈 사이드바에서 JWT 토큰을 입력하고 플랜을 조회한 후 데이터 분석을 시작해주세요.")
else:
    # 플랜 정보와 데이터 상태 표시
            ...
            with show_loading("AI 답변 생성 중..."):
                try:
                    ...
                    if response:
                        ...
                        data_analysis_available = response.get('data_analysis_available', False)
                        source_count = response.get('source_count', 0)
                        
                        if sources_found:
                            st.info(f"📋 {source_count}개의 관련 문서를 찾았습니다.")
                        if data_analysis_available:
                            st.info("📊 데이터 분석 결과가 포함되어 있습니다.")
                            
                        st.session_state.messages.append({"role": "assistant", "content": ai_response})
                    else:
                        error_msg = "AI 답변 생성에 실패했습니다. 다시 시도해주세요."
    ...
    with st.expander("📋 분석 데이터 보기"):
        if "plan_data" in st.session_state and st.session_state.plan_data:
            ...
            with col3:
                vector_status = "✅ 초기화됨" if plan_data.get('vector_store_initialized', False) else "❌ 초기화 안됨"
            ...
            if 'plan_info' in plan_data:
                ...
                with col3:
                    ...
                    gender_text = "남성" if gender == "M" else "여성" if gender == "F" else gender
            ...
            st.info("👈 아직 분석된 데이터가 없습니다. 사이드바에서 데이터 분석을 시작해주세요.")