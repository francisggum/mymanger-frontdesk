...
class DataManager:
    ...
    def process_premium_data(
        self, data: Dict[str, Any], plan_id: str, age: int, gender: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if "coverage_premiums" in data:
                ...
                if isinstance(coverage_data, list) and len(coverage_data) > 0:
                    ...
                    logger.info(
                        f"Loaded {len(filtered_coverage_data)} selected coverage premium records (filtered from {len(coverage_data)} total)"
                    )
                else:
            ...
            if "required_premiums" in data:
                ...
                if isinstance(required_data, list) and len(required_data) > 0:
                    ...
                    for item in required_data:
                        ...
                        item_copy["guide_coverage_amount"] = item_copy.get(
                            "min_insur_amount", 0
                        )
                        item_copy["guide_coverage_premium"] = item_copy.get(
                            "min_premium", 0
                        )
            ...
            if combined_coverage_data:
                ...
                logger.info(
                    f"Total combined coverage records: {len(self.coverage_premiums_df)}"
                )
            else:
            ...
            if "product_insur_premiums" in data:
                ...
                logger.info(
                    f"Loaded {len(self.product_insur_data)} product insurance records"
                )
            else:
            ...
            return {
                "status": "success",
                "coverage_count": (
                    len(self.coverage_premiums_df)
                    if self.coverage_premiums_df is not None
                    else 0
                ),
                "insurance_count": len(self.product_insur_data),
                "plan_info": {"plan_id": plan_id, "age": age, "gender": gender},
            }
    ...
    def get_current_session_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_coverage_data": self.coverage_premiums_df is not None
            and not self.coverage_premiums_df.empty,
        ...
        try:
            ...
            for key, group in grouped:
                ...
                if coverage_cd == "REQ_BASE":
                    result_df.loc[group.index, "adjusted_premium"] = group[
                        "premium"
                    ].iloc[0]
                    continue
                ...
                if len(group) == 1:
                    ...
                    for idx, row in group.iterrows():
                        if row["coverage_amount"] != base_amount:
                            ...
                            result_df.loc[idx, "adjusted_premium"] = round(
                                adjusted_premium, 0
                            )
                        else:
            ...
            logger.info(
                f"Coverage amount normalization completed for {len(result_df)} records"
            )
            return result_df
    ...
    def aggregate_coverage_by_code(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            aggregated = (
                df.groupby(
                    ["company_code", "company_name", "coverage_cd", "coverage_name"]
                )
                .agg(
                    {
            ...
            logger.info(
                f"Aggregated coverage by code: {len(aggregated)} groups created"
            )
            return aggregated
    ...
    def create_comparison_table(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            other_coverages = [
                idx for idx in pivot_df.index if idx not in desired_order
            ]
            ...
            # 비교 테이블 내용 로그 출력
            logger.info("=== 비교 테이블 생성 결과 ===")
            logger.info(f"테이블 크기: {pivot_df.shape}")
            logger.info("테이블 내용:")
            logger.info(f"\n{pivot_df.to_string()}")
            logger.info("컬럼(보험사):")
            for col in pivot_df.columns:
                logger.info(f"  - {col}")
            logger.info("인덱스(보장항목):")
            for idx in pivot_df.index:
                logger.info(f"  - {idx}")
            logger.info("=== 비교 테이블 내용 끝 ===")
    ...
    def process_coverage_data_for_comparison(
        ...
        try:
            ...
            return {
                ...
                "insurance_count": len(self.product_insur_data),
                "comparison_table": self._clean_for_json_serialization(
    ...
    def _clean_for_json_serialization(self, data: Any) -> Any:
        ...
        try:
            if isinstance(data, dict):
                return {
                    key: self._clean_for_json_serialization(value)
